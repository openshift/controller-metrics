...
func TestPathParse(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			path:     "/api/v1/namespaces/my-wizbang-operator/configmaps/foo-bar-baz",
			expected: "core/v1/namespaces/{NAMESPACE}/configmaps/{NAME}",
		},
		{
			name:     "core namespaced named resource with sub-resource",
			path:     "/api/v1/namespaces/my-wizbang-operator/secret/foo-bar-baz/status",
		...
		},
		{
			...
			path:     "/apis/batch/v1/namespaces/my-wizbang-operator/jobs",
			expected: "batch/v1/namespaces/{NAMESPACE}/jobs",
		},
		{
			name:     "extension namespaced named resource",
			path:     "/apis/batch/v1/namespaces/my-wizbang-operator/jobs/foo-bar-baz",
			expected: "batch/v1/namespaces/{NAMESPACE}/jobs/{NAME}",
		},
		{
			name:     "extension namespaced named resource with sub-resource",
			path:     "/apis/wizbang.managed.openshift.io/v1alpha1/namespaces/my-wizbang-operator/accountpool/foo-bar-baz/status",
			expected: "wizbang.managed.openshift.io/v1alpha1/namespaces/{NAMESPACE}/accountpool/{NAME}/status",
		},
		{
		...
		},
		{
			...
			path:     "/apis/wizbang.managed.openshift.io/v1",
			expected: "wizbang.managed.openshift.io/v1",
		},
		{
			name:     "unknown root",
